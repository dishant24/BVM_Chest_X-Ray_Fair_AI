...
if __name__ == "__main__":
    ...
    masked = False
    name = (
        f"{task}_model_training_without_earlystop_cosine_restart_lr_{random_state}"
        if training
        else f"{task}_model_testing_without_earlystop_cosine_restart_lr_{random_state}"
    )
    ...
    if training:
        ...
        if is_groupby:
            ...
            for group in race_groupby_dataset.keys():
                ...
                weights = torch.load(
                    "/deep_learning/output/Sutariya/main/chexpert/checkpoint/diagnostic/model_swa_20.pth",
                    map_location=device,
                    weights_only=True,
                )
            ...
            if task == "diagnostic":
                ...
                weights = torch.load(
                    "/deep_learning/output/Sutariya/main/chexpert/checkpoint/race/race_model_training_101.pth",
                    map_location=device,
                    weights_only=True,
                )
            ...
            test_model.load_state_dict(weights)
            model_testing(
                test_loader, test_model, labels, device, multi_label=multi_label
            )
