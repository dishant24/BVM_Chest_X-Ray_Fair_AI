...
if __name__ == '__main__':
    ...
    if training:
     ...
     'Support Devices']
            train_loader = prepare_dataloaders(training_dataset['Path'], training_dataset[labels].values, task, shuffle=True, dataset=dataset)
            val_loader = prepare_dataloaders(validation_dataset['Path'], validation_dataset[labels].values, task, shuffle=True, dataset=dataset)  
            ...
            val_data = validation_dataset[validation_dataset['race'].isin(top_races)].copy()
            ...
            train_loader = prepare_dataloaders(training_dataset['Path'], training_dataset['race_encoded'].values, task, shuffle=True, dataset=dataset)
            val_loader = prepare_dataloaders(val_data['Path'], validation_dataset['race_encoded'].values, task,  shuffle=True, dataset=dataset)
            ...
            train_loader = prepare_dataloaders(training_dataset['Path'], training_dataset['ethnicity_encoded'].values, task, shuffle=True, dataset=dataset)
            val_loader = prepare_dataloaders(validation_dataset['Path'], validation_dataset['ethnicity_encoded'].values, task,  shuffle=True, dataset=dataset)
            criterion = nn.CrossEntropyLoss()
        ...
        model_training(model, train_loader, val_loader, criterion, task, epoch, device=device, multi_label=False)
     ...
     'Support Devices']
            ...
            for group in ethnic_groupby_dataset.keys():
                assert not df.duplicated('subject_id').any(), f"Duplicate subject_ids in group {group}"
                assert not df.duplicated('Path').any(), f"Duplicate image paths in group {group}"
                test_loader = prepare_dataloaders(ethnic_groupby_dataset[group]['Path'], ethnic_groupby_dataset[group][labels].values, task, shuffle=True, dataset=dataset)
                ...
                model_testing(test_loader, test_model, task, device, multi_label=True, group_name=group)
        else:
     ...
     'Pneumothorax', 'Pleural Effusion', 'Pleural Other', 'Fracture',
     'Support Devices']
                test_loader = prepare_dataloaders(testing_dataset['Path'], testing_dataset[labels].values, task,shuffle=False, dataset=dataset)
                ...
                test_loader = prepare_dataloaders(testing_dataset['Path'], testing_dataset['race_encoded'].values,task, shuffle=False, dataset=dataset)
                ...
                test_loader = prepare_dataloaders(testing_dataset['Path'], testing_dataset['ethnicity_encoded'].values, task,shuffle=False, dataset=dataset)
                weights = torch.load('race_model_swa_1.pth', map_location=device, weights_only=True)
