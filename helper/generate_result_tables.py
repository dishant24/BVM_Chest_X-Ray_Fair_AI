...
def generate_tabel(test_loader, lung_test_loader, clahe_test_loader, weights, lung_weights, clahe_weights, device, model, labels, test_data, base_dir):
     ...
     race_auc_df = generate_race_roc_score("/deep_learning/output/Sutariya/main/mimic/checkpoints/traininig_with_auroc_stopping_cosine_label_smoothing_mimic_race_101.pth",
                                             race_loader, device, race_labels, test_data, 'macro')
     race_auc_lung_df = generate_race_roc_score("/deep_learning/output/Sutariya/main/mimic/checkpoints/traininig_with_lung_masking_preprocessing_mimic_race_101.pth",
                                                  race_lung_loader, device, race_labels, test_data, 'macro')
     race_auc_clahe_df = generate_race_roc_score("/deep_learning/output/Sutariya/main/mimic/checkpoints/traininig_with_clahe_preprocessing_mimic_race_101.pth",
                                                  race_clahe_loader, device, race_labels, test_data, 'macro')
     
     auc_df = get_auroc_by_groups(test_loader, weights, device, model, labels, test_data, 'macro', True)
     auc_lung_df = get_auroc_by_groups(lung_test_loader, lung_weights, device, model, labels, test_data, 'macro', True)
     auc_clahe_df = get_auroc_by_groups(clahe_test_loader, clahe_weights, device, model, labels, test_data, 'macro', True)
     ...
     grouped = race_auroc_df.groupby(['Race'])['ROC AUC'].mean().reset_index()

     summary_df = grouped.groupby(['Race', 'Preprocessing'])['AUROC'].agg(
                Macro_AUROC='mean',
