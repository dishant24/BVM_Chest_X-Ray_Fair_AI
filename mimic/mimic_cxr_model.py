...
if __name__ == '__main__':
    ...
    name = f'mask_model_traininig_{task}_{random_state}' if training else f'maks_model_testing_{task}_{random_state}'
    ...
    if training:
        ...
        if task == 'diagnostic':
            ...
            train_loader = prepare_mimic_dataloaders(train_data['file_path'], train_data['race_encoded'].values, train_data, masked, base_dir,shuffle=True, is_multilabel=multi_label)
            val_loader = prepare_mimic_dataloaders(val_data['file_path'], val_data['race_encoded'].values, val_data, masked, base_dir,shuffle=True, is_multilabel=multi_label)
            ...
            model = model_transfer_learning('/deep_learning/output/Sutariya/main/mimic/checkpoints/mask_model_traininig_diagnostic_101.pth', base_model, device)
        elif task == 'ethnicity':
            ...
            train_loader = prepare_mimic_dataloaders(train_data['Path'], train_data['ethnicity_encoded'].values, train_data, masked, base_dir,shuffle=True, is_multilabel=multi_label)
            val_loader = prepare_mimic_dataloaders(val_data['Path'], val_data['ethnicity_encoded'].values, val_data, masked, base_dir,shuffle=True, is_multilabel=multi_label)
            criterion = nn.CrossEntropyLoss()
        ...
        torch.save(model.state_dict(), f'/deep_learning/output/Sutariya/main/mimic/checkpoints/{name}.pth')
    
    else:

        if is_groupby:
            all_dataset = add_demographic_data(all_dataset_path, demographic_data_path)
            all_dataset.loc[all_dataset['race'].str.startswith('WHITE'), 'race'] = 'WHITE'
            all_dataset.loc[all_dataset['race'].str.startswith('BLACK'), 'race'] = 'BLACK'
            all_dataset.loc[all_dataset['race'].str.startswith('ASIAN'), 'race'] = 'ASIAN'
            ...
            assert not test_dataset.duplicated('file_path').any(), "Duplicate image paths found in test_dataset"
            ...
            for group in race_groupby_dataset.keys():
                ...
                assert not race_groupby_dataset[group].duplicated('file_path').any(), f"Duplicate image paths in group {group}"
                test_loader = prepare_mimic_dataloaders(race_groupby_dataset[group]['file_path'], race_groupby_dataset[group][labels].values, race_groupby_dataset[group], masked, base_dir, shuffle=False, is_multilabel=multi_label)
                weights = torch.load('/deep_learning/output/Sutariya/main/mimic/checkpoints/mimic_model_swa_diagnostic_101.pth', map_location=device, weights_only=True)
        ...
        if task =='diagnostic':
            test_loader = prepare_mimic_dataloaders(testing_data['file_path'], testing_data[labels].values, testing_data, masked, base_dir, shuffle=False, is_multilabel=multi_label)
            ...
            test_loader = prepare_mimic_dataloaders(testing_data['file_path'], testing_data['race_encoded'].values, testing_data, masked, base_dir,shuffle=False, is_multilabel=multi_label)
            weights = torch.load(f'/deep_learning/output/Sutariya/main/mimic/checkpoints/model_traininig_race_101.pth', map_location=device, weights_only=True)
            ...
            test_loader = prepare_mimic_dataloaders(testing_data['Path'], testing_data['ethnicity_encoded'].values, testing_data, masked, base_dir,shuffle=False, is_multilabel=multi_label)
